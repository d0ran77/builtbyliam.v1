...
import { 
  ...
  CheckCircle, Zap, Search, Target, Edit3, ImageIcon, Layout
} from 'lucide-react';
...
export default function Onboarding({ onClose }: OnboardingProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    inspiration_links: '',
    image_folder_link: '',
    domain_choice: 'client_owns',
    delivery_path: 'subscription',
    terms_agreed: false
  });
  ...
  const canGoNext = () => {
    if (step === 1) {
      if (!formData.full_name || !formData.email || !formData.phone) return false;
      if (formData.project_type === 'revamp' && !formData.current_url) return false;
    }
    if (step === 2) {
      if (!formData.business_name || !formData.value_prop || !formData.page_two || !formData.page_three) return false;
    }
    if (step === 4) {
      if (!formData.terms_agreed) return false;
    }
    return true;
  };
      ...
      >
        ...
        <div className="overflow-y-auto p-6 md:p-8 custom-scrollbar flex-1 relative">
          {isSuccess ? (
            ...
            <form name="onboarding" method="POST" data-netlify="true" netlify-honeypot="bot-field" onSubmit={handleSubmit} className="h-full flex flex-col">
                    ...
                    >
                      ...
                      <div className="space-y-4">
                        <input 
                          ...
                          className="w-full px-5 py-4 bg-neutral-50 border border-neutral-200 rounded-2xl focus:ring-2 focus:ring-neutral-900 focus:border-transparent outline-none transition-all font-sans text-sm font-medium text-neutral-900 placeholder:text-neutral-400"
                          ...
                          {formData.project_type === 'revamp' && (
                            ...
                            >
                              <input 
                                ...
                                className="w-full px-5 py-4 bg-neutral-50 border-2 border-neutral-200 rounded-2xl focus:ring-2 focus:ring-neutral-900 focus:border-transparent outline-none transition-all font-sans text-sm font-medium text-neutral-900 placeholder:text-neutral-400"
                      ...
                      <div className="space-y-6">
                          ...
                          <div className="grid grid-cols-1 gap-3">
                            ...
                            <div className="flex items-center gap-4">
                              ...
                              <input 
                                ...
                                className="flex-1 p-3.5 rounded-xl bg-white border border-neutral-200 text-xs font-bold uppercase tracking-widest focus:ring-2 focus:ring-neutral-900 outline-none font-sans text-neutral-900" 
                              />
                            </div>
                            <div className="flex items-center gap-4">
                        ...
                        <div className="space-y-4 pt-2">
                          <input 
                            ...
                            className="w-full px-5 py-4 bg-neutral-50 border border-neutral-200 rounded-2xl focus:ring-2 focus:ring-neutral-900 outline-none font-sans text-sm font-medium text-neutral-900 placeholder:text-neutral-400" 
                          />
                          <div>
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="p-6 bg-neutral-50 rounded-2xl border border-neutral-200">
                          ...
                          <input 
                            ...
                            className="w-full p-4 rounded-xl bg-white border border-neutral-200 text-xs font-bold outline-none focus:ring-2 focus:ring-neutral-900 font-mono text-neutral-900 placeholder:text-neutral-400" 
                          />
                        </div>
                      </div>
                    </motion.div>
                    ...
                    >
                      ...
                      <div className="space-y-8">
                        <div className="space-y-3">
                          ...
                          <div className="flex p-1.5 bg-neutral-100 rounded-2xl">
                            <button type="button" onClick={() => setFormData({...formData, domain_choice: 'client_owns'})} className={`flex-1 py-3 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all font-mono ${formData.domain_choice === 'client_owns' ? 'bg-white shadow-sm text-neutral-900' : 'text-neutral-500 hover:text-neutral-700'}`}>I own a domain</button>
                            <button type="button" onClick={() => setFormData({...formData, domain_choice: 'studio_manages'})} className={`flex-1 py-3 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all font-mono ${formData.domain_choice === 'studio_manages' ? 'bg-white shadow-sm text-neutral-900' : 'text-neutral-500 hover:text-neutral-700'}`}>Studio Manages</button>
                          </div>
                          <AnimatePresence>
                            {formData.domain_choice === 'studio_manages' && (
                              <motion.div 
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="space-y-3">
                            <label className={`block p-5 rounded-2xl cursor-pointer border-2 transition-all ${formData.delivery_path === 'subscription' ? 'border-neutral-900 bg-neutral-50' : 'border-neutral-100 hover:border-neutral-200 bg-white'}`}>
                              <input type="radio" name="delivery_path" value="subscription" checked={formData.delivery_path === 'subscription'} onChange={handleChange} className="hidden" />
                            ...
                            <label className={`block p-5 rounded-2xl cursor-pointer border-2 transition-all ${formData.delivery_path === 'handover' ? 'border-neutral-900 bg-neutral-50' : 'border-neutral-100 hover:border-neutral-200 bg-white'}`}>
                              <input type="radio" name="delivery_path" value="handover" checked={formData.delivery_path === 'handover'} onChange={handleChange} className="hidden" />
